...
class GeneticAlgorithm:
    def __init__(self,num_gens,population_size,filename,fitnessFunc, ranges,scales, angVels, vels, numProcess):
    ...
    def mutate(self, genome):
        ...
        mutate_amount=np.array([int(np.random.normal(0,1)), int(np.random.normal(0,1)), np.random.normal(0,0.1), np.random.normal(0,0.0005), int(np.random.normal(0,1)),int(np.random.normal(0,1))])
    ...
    def process_element(self, i, population):
        fit=self.fitnessFunc(self.scales, self.vels, self.angVels, None, returnTypes='Error', genome=population[i])
        print(i, fit)
        return fit
        # try:
        #     fit=self.fitnessFunc(self.scales, self.vels, self.angVels, savePath=None, returnTypes='Error', genome=population[i])
        #     print(i, fit)
        #     return fit
...
def runGA1D(run=False, plotting=False):

    filename=f'./Results/RandomData/GA_Experiment_Output/TuningMultiscale_velsWithSpikes.npy'
    savePath=f'./Results/RandomData/GA_Experiment_Output/GA_multiscale_velWithSpikesPlot.png'
    ...
    fitnessFunc=  headDirectionAndPlaceNoWrapNet
    ...
    if run==True:
        ...
        test_length=100
        # np.random.seed(5)
        vels=np.random.uniform(0,1,test_length) 
        for i in range(0,test_length,test_length//4):
            vels[i-2]=5
            vels[i-1]=10
            vels[i]=20
        angVels=np.random.uniform(0,np.pi/6,test_length)

        GeneticAlgorithm(num_gens,population_size,filename,fitnessFunc,ranges,scales, angVels, vels, numProcess=4).implimentGA()
